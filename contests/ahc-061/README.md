...
`model_reset_interval_steps` を正にすると、直近のresetからその env steps 以上経過したupdate後に、modelとoptimizerを初期化する。reset直前の重みは `checkpoints/step_<step>_pre_reset.pt` に保存される。`reset_reference_kl_coef` と `reset_reference_kl_decay_steps` を設定すると、reset前の方策からの forward KL を指定step数で線形に0へ減衰させる。
