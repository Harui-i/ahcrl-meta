...
# Semantic one-hot groups used by the neural encoder.  Every group uses an
# all-zero cell as an explicit ``none`` class in addition to the source planes.
FOOD_COLOR_COUNT = MAX_COLORS
SNAKE_COLOR_COUNT = MAX_COLORS
SEGMENT_START = PLANE_HEAD
SEGMENT_COUNT = 3
TARGET_COLOR_COUNT = MAX_COLORS
PREV_ACTION_COUNT = ACTION_COUNT

TYPED_NUM_PLANES = NUM_PLANES + 5
CATEGORICAL_EXCLUDED_CHANNELS = (
    tuple(range(FOOD_COLOR_START, FOOD_COLOR_START + FOOD_COLOR_COUNT))
    + tuple(range(SNAKE_COLOR_START, SNAKE_COLOR_START + SNAKE_COLOR_COUNT))
    + tuple(range(SEGMENT_START, SEGMENT_START + SEGMENT_COUNT))
    + tuple(range(TARGET_COLOR_START, TARGET_COLOR_START + TARGET_COLOR_COUNT))
    + tuple(range(PLANE_PREV_ACTION_START, PLANE_PREV_ACTION_START + PREV_ACTION_COUNT))
)
